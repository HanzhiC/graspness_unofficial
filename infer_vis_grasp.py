from graspnet_utils.data_utils import CameraInfo, create_point_cloud_from_depth_image, get_workspace_mask  # type: ignore
from graspnet_utils.collision_detector import ModelFreeCollisionDetector  # type: ignore
from graspnet_dataset.graspnet_dataset import minkowski_collate_fn
from graspnet_models.graspnet import GraspNet, pred_decode
import os
import sys
import numpy as np
import argparse
...
parser.add_argument('--checkpoint_path',
                    default='/home/wiss/chenh/hand_object_percept/egoprior-diffuser/third_party/graspness_unofficial/weights/minkuresunet_kinect.tar')
parser.add_argument(
    '--dump_dir', help='Dump dir to save outputs', default='/home/wiss/chenh/hand_object_percept/egoprior-diffuser/third_party/graspness_unofficial/logs/')
parser.add_argument('--seed_feat_dim', default=512,
                    type=int, help='Point wise feature dim')
...
def data_process():
    ...
    depth = np.array(Image.open(
        "/home/wiss/chenh/dataset/srl_kitchen/egoprior_wild_demo/depth_m3d/000003.png")) / 1000.0
    seg = (depth > 0).astype(np.float32)
    camera_info = load_json(
        os.path.join(
            "/home/wiss/chenh/dataset/srl_kitchen/egoprior_wild_demo/", "camera_intrinsic.json")
    )
    intrinsic = np.array(camera_info["intrinsic_matrix"]
